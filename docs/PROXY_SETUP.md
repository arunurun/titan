...
- **Find Breakouts** dispatches `breakout_scan.yml` (runs `python -m src.breakout_scanner`; emails the markdown report via the same SMTP secrets as sector digests; uploads `daily_breakout_report_v2.md` and `breakout_scanner_run.log` as workflow artifacts). Add `breakout_scan.yml` to `ALLOWED_WORKFLOWS` in `proxy/cloudflare-worker.js` and redeploy the Worker after merging.
- `GET /runs?limit=20`
