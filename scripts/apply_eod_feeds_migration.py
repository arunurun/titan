"""Apply the free-NSE EOD feed tables migration via the Supabase Management API.

Mirrors scripts/apply_reconcile_schema_migration.py. Requires SUPABASE_ACCESS_TOKEN
...
MIGRATION = ROOT / "sql" / "create_eod_feeds_tables.sql"
...
where table_schema = 'public'
  and table_name in (
    'delivery_daily','fno_ban_daily','futures_daily',
    'institutional_flow','india_vix_daily','corporate_actions_calendar',
    'eod_feed_ingest_log'
  )
order by table_name;
"""
...
def _run_management_query(project_ref: str, access_token: str, query: str) -> list | dict:
    headers = {
        ...
        "User-Agent": "Titan-EODFeeds-Migration/1.0",
...
def main() -> int:
    ...
    if not access_token:
        print(
            ...
            "Set a Supabase management token (or run sql/create_eod_feeds_tables.sql in the "
            "Supabase SQL editor) then re-run the ingestion scripts.",
            file=sys.stderr,
        )
        return 1
    ...
    print(f"Verified {len(rows)} feed table(s): {json.dumps(rows, default=str)}")
    if len(rows) < 7:
        print(f"Expected 7 feed tables; got {len(rows)}. Re-run or check schema.", file=sys.stderr)
        return 1
    return 0
