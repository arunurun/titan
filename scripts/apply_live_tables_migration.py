"""Apply live-stream table DDL via the Supabase Management API.

Optional — only runs when SUPABASE_ACCESS_TOKEN is set. Mirrors
scripts/apply_eod_feeds_migration.py.
"""
...
MIGRATION = ROOT / "sql" / "create_live_tables.sql"

VERIFY_TABLES_SQL = """
select table_name
from information_schema.tables
where table_schema = 'public'
  and table_name in ('live_regime_snapshots')
order by table_name;
"""
...
def _run_management_query(project_ref: str, access_token: str, query: str) -> list | dict:
    headers = {
        ...
        "User-Agent": "Titan-LiveTables-Migration/1.0",
...
def main() -> int:
    ...
    if not access_token:
        print(
            ...
            "Set a Supabase management token (or run sql/create_live_tables.sql in the "
            "Supabase SQL editor) then start the live consumer.",
            file=sys.stderr,
        )
        return 1
    ...
    print(f"Verified live table(s): {json.dumps(rows, default=str)}")
    if len(rows) < 1:
        print("Expected live_regime_snapshots; table not found after migration.", file=sys.stderr)
        return 1
    return 0
