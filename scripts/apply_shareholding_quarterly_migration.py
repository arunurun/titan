"""Apply the shareholding_quarterly table migration via the Supabase Management API.

Mirrors scripts/apply_eod_feeds_migration.py. Requires SUPABASE_ACCESS_TOKEN
...
MIGRATION = ROOT / "sql" / "create_shareholding_quarterly.sql"

VERIFY_TABLES_SQL = """
select table_name
from information_schema.tables
where table_schema = 'public'
  and table_name = 'shareholding_quarterly';
"""
...
def _run_management_query(project_ref: str, access_token: str, query: str) -> list | dict:
    headers = {
        ...
        "User-Agent": "Titan-Shareholding-Migration/1.0",
...
def main() -> int:
    ...
    if not access_token:
        print(
            ...
            "Set a Supabase management token (or run sql/create_shareholding_quarterly.sql in the "
            "Supabase SQL editor) then re-run scripts/ingest_shareholding_quarterly.py.",
            file=sys.stderr,
        )
        return 1
    ...
    print(f"Verified shareholding_quarterly: {json.dumps(rows, default=str)}")
    if len(rows) < 1:
        print("Expected shareholding_quarterly table; re-run or check schema.", file=sys.stderr)
        return 1
    return 0
