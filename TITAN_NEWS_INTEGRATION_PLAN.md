...
```
External Sources (NewsAPI, Finnhub, RSS, Web)
         ↓
   news_client.py (Fetcher & Normalization)
         ↓
   news_sentiment.py (VADER/FinBERT Scoring)
         ↓
   news_store.py (Supabase CRUD + Dedup)
         ↓
   news_audit.py (Quality Checks & Correlation)
         ↓
   sector_audit.py (Audit Enrichment Hook)
         ↓
   brain.py (Gemini Narrative Generation)
         ↓
   Email / X / LinkedIn Output
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ News Fetcher Thread (Scheduled: Every 2 Hours, 09:00-17:00) │
└──────────────┬────────────────────────────────────────────────┘
               │
               ├─→ NewsAPI (100 articles/day free)
               ├─→ Finnhub (real-time, Indian focus)
               └─→ RSS Feeds (Bloomberg, Reuters, Moneycontrol)
               │
               ↓
       ┌──────────────────┐
       │ Normalize Items  │
...
┌─────────────────────────────────────────────────────────────┐
│ Sector Analysis Run (Daily/On-Demand: main.py --all-sectors)│
└──────────────┬────────────────────────────────────────────────┘
               │
               ↓
       ┌──────────────────────────┐
       ...
       └────────┬───────────────────┘
                ...
                ↓
       ┌──────────────────────────────┐
       │ Call Gemini with Rich Audit  │
       │ (JSON payload includes news) │
       └────────┬─────────────────────┘
                │
                ↓
       ┌──────────────────────────────────────┐
       │ Generate Narrative:                  │
       │ - Mention top news drivers           │
       │ - Flag sentiment/price contradictions│
       │ - Note event catalysts              │
       └────────┬─────────────────────────────┘
                │
                ↓
       ┌──────────────────────────────┐
...
2. [ ] Update `requirements.txt` with news dependencies
3. [ ] Add `.py` modules to `src/`: `news_client.py`, `news_sentiment.py`, `news_store.py`, `news_audit.py`
4. [ ] Add helper scripts to `scripts/`: `fetch_news_batch.py`, `cleanup_news.py`
5. [ ] Create GitHub Actions workflow: `.github/workflows/news_fetch.yml`
6. [ ] Modify `sector_audit.py` to include news enrichment hook
7. [ ] Modify `brain.py` system prompt
8. [ ] Add tests to `tests/`
9. [ ] Update `.env.example` and documentation
10. [ ] Test in GitHub Actions (dry-run with limited symbols)
11. [ ] Monitor logs for errors, adjust `TITAN_NEWS_BLEND_WEIGHT` based on live performance
12. [ ] Enable news fetcher on production schedule
...
1. Create a function in `news_client.py`:
   ```python
...
2. Call it from `fetch_all_news_for_symbol()`:
   ```python
...
3. Add credentials to GitHub Actions & `.env.example`
