...
def test_apply_survivorship_haircut():
    summary = {"hit_rate_t10": 40.0, "hit_rate_t20": 20.0}
    adjusted = _apply_survivorship_haircut(summary)
    factor = 1.0 - SURVIVORSHIP_HAIRCUT_PCT / 100.0
    assert adjusted["expected_hit_rate_t10"] == round(40.0 * factor, 2)
    assert adjusted["expected_hit_rate_t20"] == round(20.0 * factor, 2)


def test_build_report_markdown_includes_survivorship_section():
    report = {
        "meta": {"range": "6m", "forward_horizons": [10, 20, 30]},
        "summary": {
            "total_pass_signals": 10,
            "total_watch_signals": 2,
            "pass": {
                "total_signals": 10,
                "hit_rate_t10": 40.0,
                "coverage_t10": 10,
            ...
            "watch": {"total_signals": 2},
            "survivorship_haircut": _apply_survivorship_haircut({"hit_rate_t10": 40.0}),
    ...
    assert "PASS signals" in md
    assert "WATCH signals" in md
