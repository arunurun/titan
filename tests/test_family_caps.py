...
def _enforce_caps(monkeypatch):
    monkeypatch.setenv("TITAN_ENABLE_FAMILY_CAPS", "1")
    monkeypatch.setenv("TITAN_FAMILY_CAPS_MODE", "enforce")
...
def test_legacy_mode_uncapped_sum(monkeypatch):
    monkeypatch.delenv("TITAN_ENABLE_FAMILY_CAPS", raising=False)
    audit: dict = {}
    fam = {"horizon": 1.0, "intent": 1.0, "z": 2.0, "trend": 2.0, "momentum": 3.0, "volatility": 0.0}
    c = {
        "families": fam,
    ...
    d = {"mult_momentum": 1.0, "mult_money_flow": 1.0, "mult_over_extension": 1.0, "mult_risk": 1.0}
    agg = v2._aggregate(c, d, audit)
    assert agg["risk_c"] > 4.0
    assert "family_caps" not in audit
